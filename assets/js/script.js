$(document).ready(function () {
        ...
        if (searchType === "tracks") {
            ...
            fetch(spotifyURL,URLs[0]).then(function(response){
                ...
                for(var i=0; i<spotifyData.tracks.items.length; i++) { 
                    ...
                    $(spotifyLink).addClass("spotify-link")
                    $(youtubeLink).addClass("youtube-link")
    ...
    function searchBy(){
     ...
     // it also stores values of each video liked on local storage
     $(document).on("click", ".like-heart", function (event) {
        if ($(this).hasClass("fa-solid")) {
            ...
            artistName = $(event.currentTarget).closest(".result-container").find(".artist-name"),
            songName = $(event.currentTarget).closest(".result-container").find(".song-name"),
            coverArt = $(event.currentTarget).closest(".result-container").find(".cover-art"),
            spotifyLink = $(event.currentTarget).closest(".result-container").find(".spotify-link"),
            youtubeLink = $(event.currentTarget).closest(".result-container").find(".youtube-link"),

            songData = {
                artist: artistName.html(),
                title: songName.html(),
                cover: coverArt.attr("src"),
                spotify: spotifyLink.attr("href"),
                youtube: youtubeLink.attr("href"),
            }

            faveSongs.push(songData);
    ...
    $(document).on("click", ".like-star", function (event) {
        if ($(this).hasClass("fa-solid")) {
            ...
            artistName = $(event.currentTarget).closest(".result-container").find(".channel-name"),
            songName = $(event.currentTarget).closest(".result-container").find(".video-title"),
            coverArt = $(event.currentTarget).closest(".result-container").find(".thumbnail"),
            spotifyLink = $(event.currentTarget).closest(".result-container").find(".spotify-link"),
            youtubeLink = $(event.currentTarget).closest(".result-container").find(".youtube-link"),

            vidData = {
                channel: artistName.html(),
                title: songName.html(),
                thumbnail: coverArt.attr("src"),
                spotify: spotifyLink.attr("href"),
                youtube: youtubeLink.attr("href"),
            }

            faveVids.push(vidData);
            localStorage.setItem("faveVids", JSON.stringify(faveVids));
    ...
    // when clicking on the heart in the nav bar, it will show your fave songs
    $(document).on("click", ".fave-heart", function (event) {
        mainSec.innerHTML = "";

        // gets song data from local storage
        var showSongs = JSON.parse(localStorage.getItem("faveSongs"));
        ...
        for(var i=0; i<showSongs.length; i++) { 
            ...
            $(heartDefault).addClass("fa-solid");
            $(heartDefault).addClass("fa-heart");
            $(heartDefault).addClass("like-heart");
            $(heartDefault).addClass("liked-song");

            // all links will be opened in a separate tab
            $("a").attr("target","_blank");

            // defining the content within each created element
            // the data is stored in the songData array within the faveSongs object
            artistName.innerHTML = showSongs[i].artist;
            songName.innerHTML = showSongs[i].title;
            coverArt.src = showSongs[i].cover;
            spotifyLink.href = showSongs[i].spotify;
            youtubeLink.href = showSongs[i].youtube;
      ...
      // when clicking on the star in the nav bar, it will show your fave songs
      $(document).on("click", ".fave-star", function (event) {
        mainSec.innerHTML = "";

        // gets song data from local storage
        var showVids = JSON.parse(localStorage.getItem("faveVids"));

        for(var i=0; i<showVids.length; i++) { 
            ...
            $(starDefault).addClass("fa-solid");
            $(starDefault).addClass("fa-star");
            $(starDefault).addClass("like-star");
            $(starDefault).addClass("liked-video");

            // all links will be opened in a separate tab
            $("a").attr("target","_blank");

            // defining the content within each created element
            // the data is stored in the vidData array within the faveVids object
            artistName.innerHTML = showVids[i].channel;
            songName.innerHTML = showVids[i].title;
            coverArt.src = showVids[i].thumbnail;
            spotifyLink.href = showVids[i].spotify;
            youtubeLink.href = showVids[i].youtube;
